    ...
    try {
      ...
      if (result.valid) {
        try {
          ...
          toast({ title: "Connected!", description: "API Keys are valid." });
        } catch(e) {
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent className="bg-slate-900 border-slate-700 text-white">
              ...
              <div className="space-y-4 py-4">
                 ...
                 <select className="w-full bg-[#0F1419] p-2 rounded border border-slate-800 text-white" value={formData.mode} onChange={e => setFormData({...formData, mode: e.target.value})}>
                     <option value="Live">Live Trading</option><option value="Demo">Demo / Paper</option>
                 </select>
                 ...
                 <div className="flex items-center gap-2">
                     <Button onClick={handleTestConnection} disabled={isTesting} variant="outline" className="flex-1 border-slate-600">
                         {isTesting ? <RefreshCw className="animate-spin h-4 w-4"/> : 'Test Connection'}
                     </Button>
                     <Button onClick={handleSave} disabled={formData.status !== 'active'} className="flex-1 bg-green-600 hover:bg-green-700">Save</Button>
                 </div>
              </div>
            </DialogContent>
          </Dialog>
